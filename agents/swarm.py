...
class AgentSwarm:
    ...
    def _init_agents(self, num_agents: int):
        ...
        archetypes = ["Startup Caotica", "Corporate Tossica", "Azienda Familiare", "Consulting"]
    ...
    def get_available_agents(self, human_id: str) -> List[Dict]:
        ...
        for agent_id, agent in self.agents.items():
            ...
            available.append({
                ...
                "current_day": agent.engine.player.days_survived if agent.engine else 0,
                ...
                "current_stats": agent.engine.player.to_dict()["stats"] if agent.engine else None,
                "current_factions": dict(agent.engine.player.factions) if agent.engine else None,
                "match_score": self._calculate_match_score(human_id, agent) if human_id in self.humans else 50
            })
    ...
    def possess_agent(self, human_id: str, agent_id: str, reason: Optional[str] = None) -> Dict:
        """
    ...
    def get_laboratory_view(self, human_id: Optional[str] = None) -> Dict:
        ...
        for agent_id, agent in self.agents.items():
            ...
            agents_view.append({
                ...
                "current_event": agent.engine.current_event.id if agent.engine.current_event else None,
                "match_score": self._calculate_match_score(human_id, agent) if human_id else 50
            })
        ...
        return {
            ...
            "agents": agents_view,
            "human_profile": self.humans[human_id].get_emergent_profile() if human_id and human_id in self.humans else None
        }
    ...
    def get_swarm_analytics(self) -> Dict:
        ...
        avg_stress = sum(a.engine.player.stress for a in self.agents.values() if a.engine) / len(self.agents)

        # Archetipo più stressante
        archetype_stress = {}
        for a in self.agents.values():
            if not a.engine: continue
            arch = a.company_type
            if arch not in archetype_stress:
                archetype_stress[arch] = []
            archetype_stress[arch].append(a.engine.player.stress)

        most_stressful = max(archetype_stress.items(), key=lambda x: sum(x[1])/len(x[1])) if archetype_stress else (None, 0)

        return {
            "total_decisions": total_decisions,
            "avg_stress": round(avg_stress, 1),
            "most_stressful_archetype": most_stressful[0],
            "archetype_avg_stress": {k: round(sum(v)/len(v), 1) for k, v in archetype_stress.items()},
            "possessed_count": sum(1 for a in self.agents.values() if a.is_possessed),
            "alive_count": sum(1 for a in self.agents.values() if a.engine and a.engine.player.is_alive)
        }
