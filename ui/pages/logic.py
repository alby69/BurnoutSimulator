...
def on_start_cb(name, arch, hr_params, go_to_lab=False, skip_tutorial=False):
    if go_to_lab:
        ...
        state.engine = GameEngine(name, "game/data/events.json", company_type=arch, hr_params=hr_params)
        state.session_id = str(uuid.uuid4())
        create_session(state.session_id, name, arch)
        state.screen = "game"
        if not skip_tutorial: state._tutorial_active = True
    ui.navigate.to("/")
...
def continue_possession(agent_id: str):
    if agent_id not in state.swarm.agents: return
    state.current_agent_id = agent_id
...
def make_choice(idx: int, event, choice):
    ...
    record_choice(state.session_id, state.engine.player.days_survived, event.id, choice.id, choice.text, choice.category, state.stats_before, stats_after)
    state.choice_history.append({"text": choice.text, "category": choice.category})
    deltas = {k: stats_after[k] - state.stats_before[k] for k in state.stats_before if stats_after[k] != state.stats_before[k]}
    show_choice_feedback(deltas, choice.category)

def show_choice_feedback(deltas, category):
    with ui.dialog().props("persistent scale") as dialog, ui.card().classes("p-8 vn-card"):
        ...
        for key, delta in deltas.items():
            with ui.row().classes("w-full items-center justify-between px-4 py-3 rounded-xl border border-white/5"):
                ui.label(key.upper()).classes("text-[10px] font-extrabold text-gray-300")
                ui.label(f"{'+' if delta > 0 else ''}{delta}").classes("text-sm font-black")
        ...
        def advance():
            ...
            if state.engine and state.engine.is_game_over():
                ...
                if state.engine: state.engine.next_turn()
                state.screen = "game_over" if state.engine and state.engine.is_game_over() else "game"
                if state.current_agent_id: save_agent(state.swarm.agents[state.current_agent_id].to_dict())
            ui.navigate.to("/")
...
def step_simulation(steps=1):
    for _ in range(steps): state.swarm.run_simulation_step()
    ui.navigate.to("/")

def toggle_auto_simulation():
    if not hasattr(app, "auto_sim_active"): app.auto_sim_active = False
    app.auto_sim_active = not app.auto_sim_active
    if app.auto_sim_active:
        if not hasattr(app, "auto_sim_timer"):
            app.auto_sim_timer = ui.timer(30.0, lambda: (state.swarm.run_simulation_step(), ui.navigate.to("/")))
        else: app.auto_sim_timer.activate()
    else:
        if hasattr(app, "auto_sim_timer"): app.auto_sim_timer.deactivate()
    ui.navigate.to("/")

def mood_matches_agent(mood: str, agent: dict) -> bool:
    stats = agent.get("current_stats") or {}
    if not stats: return True
    if mood == "Stressato": return stats.get("stress", 0) > 50
    elif mood == "Arrabbiato": return stats.get("self_esteem", 50) < 40 or stats.get("stress", 0) > 60
    elif mood == "Stanco": return stats.get("energy", 100) < 40
    elif mood == "Motivato": return stats.get("energy", 0) > 60 and stats.get("stress", 100) < 50
    elif mood == "Cinico": return stats.get("integrity", 50) < 40 or stats.get("team_rep", 50) < 40
    elif mood == "Speranzoso": return stats.get("self_esteem", 0) > 60 and stats.get("employability", 0) > 60
    elif mood == "Confuso": return 30 <= stats.get("stress", 50) <= 70
    return True

def render_jump_dialog():
    with ui.dialog().props("maximized") as dialog:
        with ui.card().classes("w-full h-full bg-gray-900 p-8"):
            ui.label("SALTA A UN ALTRO AGENTE").classes("text-2xl font-bold text-white mb-4")
            mood = ui.select(["Stressato", "Arrabbiato", "Stanco", "Motivato", "Cinico", "Speranzoso", "Confuso"], label="Come ti senti?").classes("w-full max-w-md mb-6")
            ...
            def render_agents():
                ...
                for agent in available:
                    if agent["agent_id"] == state.current_agent_id: continue
                    if mood.value and not mood_matches_agent(mood.value, agent): continue
                    with agents_container, ui.card().classes("w-full p-4 mb-2 bg-white/5 border border-white/10"):
                        with ui.row().classes("w-full justify-between items-center"):
                            ui.label(agent["name"]).classes("font-bold")
                            ui.button("SALTA QUI", on_click=lambda aid=agent["agent_id"], m=mood: execute_jump(aid, m.value, dialog)).props("color=purple")
...
def execute_jump(aid, mood_val, dialog):
    res = state.swarm.possess_agent(state.current_human_id, aid, reason=f"Salto emotivo: {mood_val}" if mood_val else None)
    if res.get("success"):
...
def show_help():
    with ui.dialog().props("maximized") as d:
        with ui.card().classes("w-full h-full bg-gray-900 text-white p-8 overflow-y-auto"):
            ui.label("STRATEGIC LABORATORY & DSS HELP").classes("text-2xl font-black text-blue-400 mb-6")
            ui.markdown("""
...
def show_config():
    with ui.dialog() as d, ui.card().classes("p-8 vn-card"):
        ...
        ui.label("SALVATAGGI MULTI-SLOT").classes("text-xs font-black text-blue-300 mb-2")
        with ui.row().classes("gap-4 mb-4"):
            for i in range(1, 4):
                with ui.column().classes("items-center"):
                    ui.label(f"Slot {i}").classes("text-xs")
                    ui.button(icon="save", on_click=lambda i=i: save_to_slot(i)).props("flat round color=blue")
                    ui.button(icon="upload", on_click=lambda i=i: load_from_slot(i)).props("flat round color=gray")
...
def show_decision_graph():
    ...
    for entry in history:
        ev_id, ch_id, nxt = entry["event_id"], entry["choice_id"], entry.get("next_event_id")
        if ev_id not in nodes_map: nodes_map[ev_id] = {"id": ev_id, "name": ev_id.replace("_", " ")[:15], "symbolSize": 20}
        if nxt:
            if nxt not in nodes_map: nodes_map[nxt] = {"id": nxt, "name": nxt.replace("_", " ")[:15], "symbolSize": 15}
            edges.append({"source": ev_id, "target": nxt, "lineStyle": {"curveness": 0.2}})

    option = {"series": [{"type": "graph", "layout": "force", "data": list(nodes_map.values()), "edges": edges, "roam": True, "label": {"show": True, "position": "bottom", "fontSize": 10}}], "backgroundColor": "transparent"}
    with ui.dialog() as d, ui.card().classes("w-full max-w-4xl p-4 vn-card"):
        ui.echart(option).classes("w-full h-[500px]")
    d.open()

def export_report():
    if not state.engine: return
    ...
    for tag, val in p.tags.items(): report += f"- {tag}: {val}\n"
...
def load_from_slot(slot: int):
    ui.notify("Caricamento multi-slot non ancora integrato nella UI di ripresa", type="info")
